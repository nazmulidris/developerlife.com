...
- [Implementing async middleware](#implementing-async-middleware)
...
2. `spawn()` with `await`:
   ...
   - The async `main()` function waits after each `mw_fun.spawn()` call with the `.await` call then
     unwraps the result and prints it.
...
However, if you mix and match these locks with async code, you'll get problems and the Rust compiler
will complain. Here's the kind of issue that you might face.

1. Locking the mutex will block the thread, which is generally something you want to avoid in async
   code as it can prevent other tasks from running.
...
> 🪄 Currently async lambdas are only supported in Rust nightly channel, after enabling the feature
> `async_closure`. Please see this
