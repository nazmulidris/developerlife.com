...
- [Make major changes to create-ink-app v2.1.1 scaffolding](#make-major-changes-to-create-ink-app-v211-scaffolding)
  ...
  - [Break all the things 💣 - Switch to ESM & TS strict mode](#break-all-the-things----switch-to-esm--ts-strict-mode)
...
2. **"Thing" #2 🧙 - Two symlinks created for your module**
   ...
   <pre><span style="color:#9FDA05">a</span><span style="color:#A3D704">l</span><span style="color:#A7D403">l</span><span style="color:#ABD102">-</span><span style="color:#AFCE02">t</span><span style="color:#B3CA01">h</span><span style="color:#B7C701">e</span><span style="color:#BBC301">-</span><span style="color:#BEC001">p</span><span style="color:#C2BC01">a</span><span style="color:#C5B801">c</span><span style="color:#C9B401">k</span><span style="color:#CCB102">a</span><span style="color:#D0AD02">g</span><span style="color:#D3A903">e</span><span style="color:#D6A504">-</span><span style="color:#D9A105">n</span><span style="color:#DC9C06">a</span><span style="color:#DF9807">m</span><span style="color:#E29409">e</span><span style="color:#E4900A">s</span>
...
<pre>Hello, <span style="color:#A3BE8C">Grogu</span></pre>
...
1. Notably, we are going to drop `ava` v4.x for testing because it doesn't have proper support for
   native Node.js ECMAScript modules (ESM), which we will be using for our node module.
2. We are also going to drop `xo`.
3. We are also going to change some folder names.
...
> 💡 You can run `npm outdated` to see a what dependencies in your `package.json` can be updated.

### Break all the things 💣 - Switch to ESM & TS strict mode

This is going to be a **huge divergence** to the configuration and code that's generated by the
scaffolding. `CommonJs` is used as the module format which means that `require` statements and
`module.exports` are required. You can see this in the TypeScript files generated in the `source`
folder.

> 💡 This is a great [tutorials](https://2ality.com/2021/06/typescript-esm-nodejs.html) that walks
> you thru enabling native ESM on Node.js.

> ⚠ Currently (as of Oct 2021) many libraries, like Jest or ava tend to use `require` at their boot
> up sequence assuming their config files are written in CJS (CommonJS modules) which then fails
> because we are using TS files which are compiled to EJM (which means `require` can't be used).
> Yikes! 😮

Here are the changes that need to be made to `tsconfig.json`.

```json
{
  "extends": "@sindresorhus/tsconfig",
  "compilerOptions": {
    "module": "ES2020", // Generate ECMAScript modules.
    "moduleResolution": "Node", // Module resolution for Node.
    "allowSyntheticDefaultImports": true, // Allow legacy CommonJS modules to be imported.
    "strict": true, // Enable strict mode in TypeScript.
    "jsx": "react",
...
Here are the changes that need to be made the `src/cli.tsx` file.

- In `cli.tsx` the call to `meow()` has to be changed thanks to strict mode in TypeScript.
...
- Also in `cli.tsx` the import statement for `ui.tsx` has to be changed to
  `import App from "./ui.js"` thanks to the use of native ESM in Node.js.

  > 💡 The Node.js [official docs](https://nodejs.org/api/esm.html#mandatory-file-extensions) shed
  > more light on why `tsx` or `ts` files have to be imported w/ a `js` extension.

Here is a change that need to be made to the `src/ui.tsx` file.

- There's a `module.exports = App` line that needs to be deleted. We are now only using
  `export default App` thanks to native ESM support in Node.js.
...
By running `npm uninstall -D ava @ava/typescript` we drop support for `ava` test runner. As of Ava
v4.x it really doesn't have proper support for ESM in Node.js, and `import` statements that are used
in the test runner implementation itself which are not possible to get around, since we want to
unify everything to use `import` that we write.

Delete the`src/test.tsx` file as well, since it will no longer compile (not that it could even run
w/ native Node.js ESM support).
